...
import { Search, Plus, User, FileText, Calendar, Trash2, Loader, LayoutGrid, List } from 'lucide-react';
                    ...
                    <tr key={p.id} className="hover:bg-slate-50 transition-colors cursor-pointer group" onClick={() => navigate(`/patients/${p.id}`)}>
                       <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-bold text-slate-800 group-hover:text-blue-900">{p.name}</span>
                          </div>
                       </td>
                       <td className="px-6 py-4 font-mono text-slate-600">{p.cpf || '-'}</td>
                       <td className="px-6 py-4 text-slate-600">{p.dob ? new Date().getFullYear() - new Date(p.dob).getFullYear() : '-'}</td>
